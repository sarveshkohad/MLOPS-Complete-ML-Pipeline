...
def transform_text(text):
    """
    Transform the imput text by converting it into lower case, tokenizing, removing stopwords
    """
    ps = PorterStemmer()
...
def preprocess_df(df, text_column = 'text', target_column = 'target'):
    """"
    Preprocesses the Dataframe by encoding the target column, removing the duplicates, and transforming the text column.
    """
    try:
        ...
        # Remone the duplicates rows
        df = df.drop_duplicates(keep = 'first')
        logger.debug("Duplicates removed")

        # apply the  transformation to the specific text column
